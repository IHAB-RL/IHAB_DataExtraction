...
dataFileName = [ szTestSubject '_FinalDat_' num2str(day(desiredDay)) '_'...
...
figure('Units','normalized','Position',[0.15 0.05 0.5 0.85]);
...
set(axOVD,'XTick',datenum(YTicksTime));
set(axRMS,'XTick',datenum(YTicksTime));
...
for ss = 1:length(FinalTimeQ)
    ...
    ylim([0.95 1.15]);
%     ActivityDescription = {'Ausruhen','Essen','Küchenarbeit',...
%     'Lesen Computer', 'Musik hören', 'Hausarbeit sonst' , ...
%     'Garten Balkon' , 'Auto selbst' , 'Auto mit' , ...
%     'Bus' , 'Bahn', 'ZuFuß', 'Fahrrad', ...
%     'Besuchen', 'Feier' ,'Restaurant', 'Theater etc', ...
%     'Versammlung' , 'Behörde Praxis' , 'Geschäft', ...
%     'Büro' , 'Werkstatt' , 'Schalter', 'Besprechungsraum', ...
%     'Draußen', 'Kantine', 'Sonstiges'};
    ...
    set(axQ,'YTick',[]);
    %set (axQ,'XTick',[]);
    set(axQ,'XTick',datenum(YTicksTime));
    set(axQ,'XTickLabel',[]);    
    xlim([datenum(FinaltimeVecPSD(1)) datenum(FinaltimeVecPSD(end))]);
...
% EXECUTE.
% 
% * Korrekturwerte PDF per Mail von Jörg am 12.9.2016
% 
