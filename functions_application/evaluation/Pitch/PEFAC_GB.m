function [fx,t,pv,fv]=PEFAC_GB(Ypower,nFFT,fs,m)
%V_FXPEFAC PEFAC pitch tracker [FX,TT,PV,FV]=(S,FS,TINC,M,PP)
%
% Input:   Ypower     Power Spectral Density, time x freq
%          nFFT       size of the Fast Fourier Transform
...
%          t(nframe)     Time at the centre of each frame (seconds).
...
% tx  - time axis values (in seconds)
% f   - frequency axis values in Hz
% MIX - spectrogram values in power per x; format time, freq
...
f = fmin : fstep : fmax;   % fmin to fmax in steps of finc
nfrq = length(f); % number of frequencies

Ypower = Ypower*2/fs(1);  % Power per Hz
% correct for no negative zero frequency to double the power
Ypower(:,1) = Ypower(:,1)*0.5;
% correct for no negative nyquist frequency to double the power
Ypower(:,end) = Ypower(:,end)*0.5;
% Now map onto the desired frequency scale
fbopt = 'cush';
Ypower = Ypower*filtbankm(nfrq,nFFT,fs(1),fmin,fmax,fbopt)';

% number of frames
nBlocks = size(Ypower,1);

% calculate time vector
tinc=[p.tinc -Inf Inf];
winlen = fix(1.81*fs(1)/20);       % window length
...
t=t0+ninc*(0:(nBlocks-1)).';
t=fs(2)+(t+ix1-2)/fs(1);

% actual frame increment
txinc=t(2)-t(1);  
...
% trans - a sparse matrix containing the v_filterbank amplitudes
% cf    - the v_filterbank centre frequencies in Hz
...
% estimated ltass
O = O.*repmat(diff(auxf),nBlocks,1);     % weight spectrum by bin width
O1 = O;

if t(end)<p.shortut                        % if it is a short utterance
    ...
    % Linear AC
    alpha = repmat(ltass,nBlocks,1)./(eltass);
    O = O.*alpha;
...
ff = zeros(nBlocks,numopt);
amp = zeros(nBlocks,numopt);
for i=1:nBlocks
...
% FORWARDS
% Initialize values
cost = zeros(nBlocks,numopt);
prev = zeros(nBlocks,numopt);
medfx = zeros(nBlocks,1);
dffact=2/txinc;
...
for i=2:nBlocks              % main dynamic programming loop
    if i>inmf
...
if ~nargout || any(m=='g') || any(m=='G')
    ...
    if any(m=='G') || ~nargout && ~any(m=='g')
        ...
        hold on
        plot(t,log10(fxg),'-b',t,log10(fxb),'-r'); % fx track
        yy=get(gca,'ylim');
        plot(t,yy(1)+yy*[-1;1]*(0.02+0.05*pv),'-k'); % P(V) track
        hold off
        nax=nax+1;
        axh(nax)=gca;
        if any(m=='g')
    ...
    if any(m=='g')
        ns=length(s);
        [tsr,ix]=sort([(1:ns)/fs 0.5*(t(1:end-1)+t(2:end))']); % intermingle speech and frame boundaries
        ...
        clf;
        subplot(5,1,1);
        plot(t,pv,'-b',(1:ns)/fs,0.5*mod(cumsum(fx(sp2fr)/fs),1)-0.6,'-b');
        nax=nax+1;
        axh(nax)=gca;
        ylabel('\phi(t), P(V)');
        ...
        axh(nax)=gca;
        subplot(5,1,4:5);
        plot(t,fxg,'-b',t,fxb,'-r');
        ylabel('Pitch (Hz)');
        %         semilogy(t,fxg,'-b',t,fxb,'-r');
